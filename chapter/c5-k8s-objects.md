## K8S 오브젝트 ##
### Pod ###
쿠버네티스의 파드(Pod)란 하나 이상의 컨테이너를 논리적으로 묶어 함께 실행하고 관리하는 배포 단위로, 동일한 파드 내 컨테이너들은 IP 주소와 포트, 저장 공간을 공유한다. 파드는 쿠버네티스가 클러스터 안에서 업무를 할당할 때 사용하는 기본 명령 단위이며, 파드 자체가 문제가 생겨 사라지더라도 시스템이 스스로 새로운 파드를 만들어 복구하는 선언적 관리 모델의 핵심 객체라고 할 수 있다.
```
apiVersion: v1
kind: Pod
metadata:
