...
ALB의 가중치 기반 타겟 그룹(Weighted Target Groups) 라우팅을 활용하면 카나리 및 블루/그린 배포를 구현할 수 있다. 먼저 그라비톤 타겟그룹(tg-arm)을 생성한 후, 리스너 규칙의 가중치를 0%로 설정함으로써 기존 서비스에 영향을 주지 않고 신규 인스턴스를 등록하고 헬스 체크를 완료할 수 있다. 이후 비즈니스 요구사항에 맞춰 가중치 비율을 점진적으로 조정하여 안전하게 트래픽을 전환한다.
...
* 고정 세션: 가중치 기반 라우팅 사용 시에도 타겟 그룹 유지(Target Group Stickiness) 설정을 통해 사용자가 동일한 버전의 애플리케이션에 계속 머물도록 보장할 수 있다.

EC2 콘솔에서 로드밸런서 메뉴로 이동하여 my-alb 를 선택한다. 
...
USER_DATA_BASE64=$(base64 -i ~/get-started-eks/ec2/cf/monte-carlo.sh | tr -d '\n')
...
EC2 콘솔의 로드밸런서 메뉴에서 my-alb 정보를 조회한다. 
